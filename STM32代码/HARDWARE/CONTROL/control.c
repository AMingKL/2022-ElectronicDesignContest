...
void EXTI9_5_IRQHandler(void)
{
	...
	if(EXTI_GetITStatus(EXTI_Line5)!=0)//一级判定
	{
		if(PBin(5)==0)//二级判定
		{
			...
			{
			 if(mpu_dmp_get_data(&pitch,&roll,&yaw)==0){}//获取当前角度						 
			}	
	...
	if (Param.ModeChoose==BACK_PACKING)
	{
		switch(Flag.Run_Step)
		{
			case 1://小车直行，同时检测串口数据等待停车
							...
							{
								Kinematic_Analysis(1000,yaw);//小车运动											
			...
			case 3://匀速后退，定时0.72s
						...
						{
							Param.Motor1_PWM_Out=-1000;
							Param.Motor2_PWM_Out=-1000;	 
							Param.Servo_Target_Val=SERVO_INIT;
			...
			case 4://右转60°，倒车，差速转弯
						if((Flag.Is_Start_Astern==1)&&(Flag.Start_Count==0))
						{
							Param.Motor1_PWM_Out=-500;
							Param.Motor2_PWM_Out=-3500;	 
							Param.Servo_Target_Val=SERVO_INIT+60.0*5;
 ...
 * 描述:小车运动分析函数
 * 输入:velocity:输入期望速度 angle:前轮当前转角
 * 输出:无
 */
void Kinematic_Analysis(float velocity,float angle)
{
	...
	{
		Param.Motor1_PWM_Out=velocity;
		Param.Motor2_PWM_Out=velocity;
	}
	//2.倒车保持俩轮差速
	else if (Flag.Is_Start_Astern==1)
	{
		Param.Motor1_PWM_Out=velocity*(1-T*tan(angle)/2/L);
		Param.Motor2_PWM_Out=velocity*(1+T*tan(angle)/2/L);	 
	}
