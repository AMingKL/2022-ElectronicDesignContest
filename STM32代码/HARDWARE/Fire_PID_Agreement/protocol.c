    ...
    switch (cmd_type)
    {
      ...
      case SET_P_I_D_CMD:
      {
        ...
        d_temp = *(float *)&temp2;
				...
				/*设置pid参数，设置哪个取消注释哪个*/
//				set_p_i_d(1,p_temp, i_temp, d_temp);//设置位置环 KP KI KD各项参数
        set_p_i_d(0,p_temp, i_temp, d_temp);//设置速度环 KP KI KD各项参数
      }
      break;
      ...
      case SET_TARGET_CMD:
      {
        ...
        /*要调哪个PID就取消哪个注释*/
//				set_pid_target(0,(float)actual_temp);    // 位置环设置目标值
        set_pid_target(0,(float)actual_temp);    // 速度环设置目标值
      ...
      {
				/*复位*/
       	__set_FAULTMASK(1);   // 关闭所有中断，防止复位被影响
				NVIC_SystemReset();   // 复位        
      }
      break;
...
//        SET_BASIC_TIM_PERIOD(temp);                          // 设置定时器周期1~1000ms
				...
				//野火的例程中，这个周期是中断调用PID计算函数的定时器的中断周期。
				//我是打算在软件定时器中调用PID的计算函数
      }
      break;
...
void set_computer_value(uint8_t cmd, uint8_t ch, void *data, uint8_t num)
{
  ...
  Usart3_SendString((uint8_t *)&set_packet, sizeof(set_packet));    // 发送数据头
	Usart3_SendString((uint8_t *)data, num);                          // 发送参数
	Usart3_SendString((uint8_t *)&sum, sizeof(sum));                  // 发送校验和
	#endif
