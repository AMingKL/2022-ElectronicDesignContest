  ...
  while(1)
  {
    ...
    switch (cmd_type)
    {
      ...
      case SET_P_I_D_CMD:
      {
        ...
        set_p_i_d(&PID,p_temp, i_temp, d_temp);//设置速度环 KP KI KD各项参数
      }
      break;
      ...
      case SET_TARGET_CMD:
      {
        int actual_temp = COMPOUND_32BIT(&frame_data[13]);    // 得到数据
				...
				set_pid_target(&PID,set_point);    // 位置环设置目标值
				LED_ON;
      }
      break;
...
void set_computer_value(uint8_t cmd, uint8_t ch, void *data, uint8_t num)
{
  ...
  Usart1_SendString((uint8_t *)&set_packet, sizeof(set_packet));    // 发送数据头
	Usart1_SendString((uint8_t *)data, num);                          // 发送参数
	Usart1_SendString((uint8_t *)&sum, sizeof(sum));                  // 发送校验和
	#endif
